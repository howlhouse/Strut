...
export default function BudgetsPage({ data, catalog, addBudget, updateBudget, deleteBudget, addTag, isAdmin }) {
  ...
  const availableTags = (catalog?.tags || []).filter((t) => !budgets.some((b) => b.tagId === t.id));

  return (
    <div>
      <PageHeader title="Budgets" subtitle="Monthly spending targets by tag" action={
      ...
      <Panel title="Budgets">
        {budgets.length === 0 ? (
          <Empty text="No budgets yet. Add a tag — like Eating out or Hobbies — and a monthly amount to start tracking it." />
        ) : (
      ...
      <p className="page-footnote">
        Transactions on the <strong>Card ledgers</strong>, <strong>Bank Accounts</strong> and <strong>Dashboard</strong> pages count against a budget when you pick that budget's tag on them.
      </p>
    </div>
  );
}
...
function BudgetRow({ budget, data, catalog, updateBudget, deleteBudget }) {
  const tag = (catalog?.tags || []).find((t) => t.id === budget.tagId);
  ...
  return (
    <div className="ledger-row wrap" style={{ flexDirection: "column", alignItems: "stretch" }}>
      <div className="ledger-row" style={{ borderTop: "none", padding: "0 0 6px" }}>
        <span className="dot" style={{ background: tag?.color || "var(--line)" }} />
        <span className="col-name">{tag ? <Pill item={tag} /> : "unknown tag"}</span>
      ...
      <div className="progress-row" style={{ marginBottom: 4 }}>
        <div className="progress-track"><div className="progress-fill" style={{ width: `${pct}%`, background: remaining < 0 ? "var(--rust)" : (tag?.color || "var(--brand)") }} /></div>
...
function AddBudgetForm({ availableTags, isAdmin, onAdd, onAddTag }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!tagId || !monthlyAmount) return;
    onAdd({ tagId, monthlyAmount: Number(monthlyAmount) });
    setMonthlyAmount("");
  };
        ...
        <Field label="Tag">
          {availableTags.length > 0 ? (
            ...
            <p className="hint" style={{ margin: 0 }}>
              {isAdmin ? "No unbudgeted tags left — add a new one below." : "No tags available yet — ask an admin to add one in the Admin Console."}
            </p>
          )}
        </Field>
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed var(--line)" }}>
          {creatingTag ? (
            <div className="add-row-form" style={{ flexWrap: "wrap" }}>
              <input className="input input-small" value={newTagName} onChange={(e) => setNewTagName(e.target.value)} placeholder="New tag name" />
              <ColorSwatchPicker value={newTagColor} onChange={setNewTagColor} />
              <button type="button" className="btn btn-primary btn-small" onClick={createTag}><Plus size={13} /> Create</button>
              <button type="button" className="btn btn-ghost btn-small" onClick={() => setCreatingTag(false)}><X size={13} /></button>
            </div>
          ) : (
            <button type="button" className="btn btn-ghost btn-small" onClick={() => setCreatingTag(true)}><Plus size={13} /> New tag</button>
          )}
        </div>
      )}
    </form>
